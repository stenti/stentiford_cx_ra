...
for av in AVs:
    frames = generate_stimuli.generate_fromAV(av,rotations=4)
    for grid_type in grid_types:
        for exp in range(n_exp):
            ...
            t_1rev = sim_len//4               # ms in 1 revolution
            ...
            R4_in[:,tm_in<(t_1rev+init)] = 0
            R2_in[:,tm_in<(t_1rev+init)] = 0

            #ANGULAR VELOCITY
            # def get_current(av):
            #     return (0.007191900905443356 * av) + (-0.00016435895992090524 * av**2) + (1.8476243529661126e-06 * av**3) + 0.005548305163772259
            ...
            for position in np.arange(n_PEN//2):
                PEN_in[position,:] = I
                PEN_in[position+8,:] = -I

            PEN_in[:,tm_in>((t_1rev*3)+init)] = 0
            PEN_in[:,tm_in>((t_1rev*3)+init)] = 0
            ...
            if learning == 'on':
                stdp_params = {"tau": 50.0,"rho": rho,"eta": eta,"wMin": -wMin,"wMax": .0, "start": t_1rev+init, "end": float(tm_in[-1])}
            else:
            ...
            data = {'Grid Type':grid_type,'AV':av,'RMSE':test_RMSE}
            dataset.append(data)
            ...
            if plot_figures:
                ...
                ax1.fill_between([tm_in[(n_1rev*3)],sim_len], -1, n_R2 + n_R4 + 1,color='green', alpha=0.1)
                ax2.fill_between([tm_in[(n_1rev*3)],sim_len], -1, n_EPG + 1,color='green', alpha=0.1)
                ax6.fill_between([tm_in[(n_1rev*3)],sim_len], -1, np.max(gt)+1,color='green', alpha=0.1)
                ax1.fill_between([0,tm_in[n_1rev]], -1, n_R2 + n_R4 + 1,color='pink', alpha=0.1)
                ax2.fill_between([0,tm_in[n_1rev]], -1, n_EPG + 1,color='pink', alpha=0.1)
                ax6.fill_between([0,tm_in[n_1rev]], -1, np.max(gt)+1,color='pink', alpha=0.1)
                ...
                ax6.plot(med_tms,gt, c = 'Black', label='Ground Truth')
                ...
                ax6.set_yticks([0,360,720,1080,1440,1800,2160])
                ax1.set_xticks([])
                ax2.set_xticks([])
                ...
                plt.savefig(f'results/{fpath}grid/{fname}_{grid_type}.png', bbox_inches='tight')
                # plt.savefig(f'results/{fpath}grid/{fname}_{grid_type}.svg', bbox_inches='tight')
                plt.close(fig)   

df = pd.DataFrame(dataset)
df.to_pickle(f'results/AVgrid/sweep_av_grids_3rev.pkl')