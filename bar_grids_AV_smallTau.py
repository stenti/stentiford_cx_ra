    ...
    for av in AVs:
        ...
        for exp in range(n_exp):
            ...
            for sh in range(n_shift):
                ...
                I = get_current(av)
                gt = np.cumsum(np.ones(n_frames) * av *(1/60)) 
                ...
                R_R_init        = params["R_R_init"]
                Rx_EPG_init     = params["Rx_EPG_init"]

                wMin = params["wmin"]
                eta = params["eta"]
                rho = params["rho"]

                GABAA_post_syn_params = {"tau": 5.0}
                NMDA_post_syn_params = {"tau": 20.0}
                ...
                if plot_figures:
                    ...
                    ax6.fill_between([tm_in[(n_1rev*2)],sim_len], -1, np.max(gt)+1,color='green', alpha=0.1)
...
df.to_pickle(f'results/AVgrid/sweep_av_grids.pkl')